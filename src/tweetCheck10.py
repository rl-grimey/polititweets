...
class tweetCheck:
    def __init__(self):
        self.userIds = []
        logging.basicConfig(filename='output10.log', level = logging.INFO)
        self.filePath = os.path.abspath(__file__).split('tweetCheck10.py')[0]
        #read in userNames
    ...
    def getUserIds(self):
        ...
        self.userIds = self.userIds[127384:141528]
    ...
    def checkBotOrNot(self):
        ...
        screennames = []
        threads = []
        path = os.path.join(self.filePath, 'tokens10.json')
    ...
    def asyncWriter(self, resultsList):
        try:
            for results in resultsList:
                ...
                print entry
                botOrNotData = {}
                path = os.path.join(self.filePath, 'botOrNotResults10.json')
                with open(path) as f:
                    try:
