...
class tweetCheck:
    def __init__(self):
        self.userIds = []
        logging.basicConfig(filename='output6.log', level = logging.INFO)
        self.filePath = os.path.abspath(__file__).split('tweetCheck6.py')[0]
        #read in userNames
    ...
    def getUserIds(self):
        ...
        self.userIds = self.userIds[70769:84922]
    ...
    def checkBotOrNot(self):
        ...
        screennames = []
        threads = []
        path = os.path.join(self.filePath, 'tokens6.json')
    ...
    def asyncWriter(self, resultsList):
        try:
            for results in resultList:
                ...
                print entry
                botOrNotData = {}
                path = os.path.join(self.filePath, 'botOrNotResults6.json')
                with open(path) as f:
                    try:
