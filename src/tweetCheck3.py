...
class tweetCheck:
    def __init__(self):
        self.userIds = []
        logging.basicConfig(filename='output3.log', level = logging.INFO)
        self.filePath = os.path.abspath(__file__).split('tweetCheck3.py')[0]
        #read in userNames
    ...
    def getUserIds(self):
        ...
        self.userIds = self.userIds[28307:42460]
    ...
    def checkBotOrNot(self):
        ...
        screennames = []
        threads = []
        path = os.path.join(self.filePath, 'tokens3.json')
    ...
    def asyncWriter(self, resultsList):
        try:
            for results in resultsList:
                ...
                print entry
                botOrNotData = {}
                path = os.path.join(self.filePath, 'botOrNotResults3.json')
                with open(path) as f:
                    try:
