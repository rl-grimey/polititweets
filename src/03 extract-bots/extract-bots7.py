...
class tweetParser:
	...
	def getBots(self, botsFile):
		bots = pd.read_csv(botsFile, usecols=['userID'])
		botSet = set(bots['userID'].tolist())
		return botSet
	...
	def aggregateFile(self, fName, aggFolder):
		...
		bots = pd.read_csv('bot-ids.csv', usecols=['userID'], dtype={'userID': 'object'})
		botSet = set(bots['userID'].tolist())
