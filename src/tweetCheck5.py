...
class tweetCheck:
    def __init__(self):
        self.userIds = []
        logging.basicConfig(filename='output5.log', level = logging.INFO)
        self.filePath = os.path.abspath(__file__).split('tweetCheck5.py')[0]
        #read in userNames
    ...
    def getUserIds(self):
        ...
        self.userIds = self.userIds[56615:70768]
    ...
    def checkBotOrNot(self):
        ...
        screennames = []
        threads = []
        path = os.path.join(self.filePath, 'tokens5.json')
    ...
    def asyncWriter(self, resultsList):
        try:
            for results in resultsList:
                ...
                botOrNotData = {}
                path = os.path.join(self.filePath, 'botOrNotResults5.json')
                with open(path) as f:
                    try:
