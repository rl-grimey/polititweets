...
class tweetCheck:
    def __init__(self):
        self.userIds = []
        logging.basicConfig(filename='output8.log', level = logging.INFO)
        self.filePath = os.path.abspath(__file__).split('tweetCheck8.py')[0]
        #read in userNames
    ...
    def getUserIds(self):
        ...
        self.userIds = self.userIds[99077:113229]
    ...
    def checkBotOrNot(self):
        ...
        screennames = []
        threads = []
        path = os.path.join(self.filePath, 'tokens8.json')
    ...
    def asyncWriter(self, resultsList):
        try:
            for results in resultsList:
                ...
                print entry
                botOrNotData = {}
                path = os.path.join(self.filePath, 'botOrNotResults8.json')
                with open(path) as f:
                    try:
